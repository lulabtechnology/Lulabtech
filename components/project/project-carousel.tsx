...
export function ProjectCarousel({ title, description, items }: ProjectCarouselProps) {
  ...
  const onSelect = useCallback(() => {
    ...
    setSnapCount(emblaApi.scrollSnapList().length || items.length);
  ...
  return (
    <div className="rounded-[34px] border border-slate-200 bg-white p-4 shadow-soft sm:p-5 lg:p-7">
      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="-ml-4 flex">
          {items.map((item) => (
            <div
              key={`${item.categoryLabel ?? title}-${item.title}`}
              className="min-w-0 flex-[0_0_100%] pl-4 sm:flex-[0_0_82%] md:flex-[0_0_62%] lg:flex-[0_0_48%] xl:flex-[0_0_38%]"
            >
              <ProjectCard item={item} categoryLabel={item.categoryLabel ?? title} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {Array.from({ length: Math.max(snapCount, 1) }).map((_, index) => (
          <button
            key={index}
            type="button"
